...
export default function Slideshow(props) {
  ...
  const childrenRef = useMemo(() => {
    return slots.map((index) => createRef())
  }, [slots])
  ...
  const [shouldPlayOnHover, setShouldPlayOnHover] =
    useState(autoPlayControl) /* For cursor updates */
  ...
  const canvasPosition = isHorizontal
    ...
    : -startFrom *
      ((size === null || size === void 0 ? void 0 : size.itemHeight) +
        gap) /* Calculate the new value to animate to */
  const newPosition = () =>
    factor *
    currentItem *
    itemWithGap /* Wrapped values for infinite looping */ /* Instead of 0 to a negative full duplicated row, we start with an offset */
  ...
  const wrappedIndexInverted = wrap(
    0,
    -totalItems,
    currentItem
  ) /* Update x or y with the provided starting point */ /* The subtraction of a full row of children is for overflow */
  useLayoutEffect(() => {
    if ((size === null || size === void 0 ? void 0 : size.children) === null)
      return /* Initial measure */ // if (initialResize.current) {
  ...
  const handleDragEnd = (event, { offset, velocity }) => {
    ...
    const isHalfOfPrev =
      offsetXorY >
      size.item / 2 /* In case you drag more than 1 item left or right */
    const normalizedOffset = Math.abs(offsetXorY)
    const itemDelta = Math.round(
      normalizedOffset / size.item
    ) /* Minimum delta is 1 to initiate a page switch */ /* For velocity use only */
    const itemDeltaFromOne =
      itemDelta === 0
        ? 1
        : itemDelta /* For quick flicks, even with low offsets */
...
const clamp = (num, min, max) =>
  Math.min(Math.max(num, min), max) /* Slide Component */
...
function Dot({
  ...
  /* Check active item */ /* Go 0—1—2—3—4—5—0 */ let isSelected =
    wrappedIndex === index /* Go 0—5—4—3—2—1—0—5 instead when inverted */
  if (isInverted) {
